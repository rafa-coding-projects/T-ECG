% genTransform4noisy.m
%
% This functions finds a transformation signal, transformedSig, that
% transforms noisy to base_sig. The signal transformedSig can be
...
%   noisy - noisy signal
%   candidate_sig -  signal reference to the noisy one
% Outputs:
%   T - Transformation signal
%   tmpBase_sum - Template for baseline signal, (it can be used for SNR)
% Date: Apr. 10th, 2020
% Author: Rafael L. da Silva
function [T, tmpBase_sum] = genTransform4noisy(base_sig, candidate_sig, noisy, pk_base,pk_cand)
    ...
    for k=2:length(pks_base)
        ...
        % Now check noisy signal
        if size(noisy(locs_cand(k-1):locs_cand(k)),1) > avg
            tmpCand = resample(noisy(locs_cand(k-1):locs_cand(k)), avg,...
                size(noisy(locs_cand(k-1):locs_cand(k)),1));
        elseif size(noisy(locs_cand(k-1):locs_cand(k)),1) < avg
            tmpCand = resample(noisy(locs_cand(k-1):locs_cand(k)), avg, ...
                size(noisy(locs_cand(k-1):locs_cand(k)),1));
        else
            tmpCand = noisy(locs_cand(k-1):locs_cand(k));
        end
